		...
		switch(bf_code[i]){
			...
			case ']':
				if(stack_offset[nested_loop_count - 1]){
					...
					stack_offset[nested_loop_count - 1] = 0;
					nested_loop_count--;
				}
				i++;
				break;
...
void BrainFuckVM::run()
{
	...
	memset(this->memory,0,MEMORY_MAX);
	
	// running code
	mprotect(this->jit_page, this->jit_page_size, PROT_READ | PROT_EXEC);
	jit_run(this->memory);
	mprotect(this->jit_page, this->jit_page_size, PROT_READ | PROT_WRITE);
